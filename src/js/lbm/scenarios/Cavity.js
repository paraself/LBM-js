function Cavity() {
	...
	this.init_cells = function (cells) {
		...
		var flow_vel = new Vec2(this.options.flow_vel, 0);
		...
		for (var c = 0; c < cells.length; c++) {
			...
			// top flow
			for (var i = 0; i < 9; i++) {
				cells[c][0][i] = get_equi(i, this.options.init_density, flow_vel, v_dot_v);
			}

			// the rest
			for (var r = 1; r < cells[c].length; r++) {
				for (i = 0; i < 9; i++) {
					cells[c][r][i] = get_equi(i, this.options.init_density, zero_vel, 0);
				}
			}
		}
	};
